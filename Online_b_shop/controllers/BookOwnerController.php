<?php
	...
	function insertUser($name,$uname,$pass,$nid,$num){
		$query = "insert into owner values (NULL,'$name','$uname','$pass','$nid','$num')";
		return execute($query);
		
	}
	function authenticateUser($uname,$pass){
		$query = "select * from owner where username='$uname' and password='$pass'";
		$rs = get($query);
		if(count($rs) > 0){
			return true;
		}
	...
	{
		$query="update owner set name='$name',username='$uname',password='$pass',nid='$nid',num='$num' where id= '$id'";
		return execute($query);
